...
def train_agent_batch(
    ...
    blank_interval = 10000

    eval_stats_history = []  # List of evaluation episode stats dict
    try:
        while True:
            if t % blank_interval == 0 and blank_env is not None:
                fire_blanks(agent, blank_env, max_episode_len)
...
def fire_blanks(agent, blank_env, max_episode_len):
    print("optimizing discriminator further...")
    blank_episode_idx = np.zeros(blank_env.env.num_envs, dtype="i")
    blank_episode_len = np.zeros(blank_env.env.num_envs, dtype="i")
    have_reset = np.zeros((blank_env.n_skills,))

    initial_z = blank_env.env.num_envs
    obss = blank_env.reset(None, force_z=np.arange(initial_z))

    while True:
        if False not in (have_reset > 0):
            print("Done optimizing discriminator further")
            return

        with torch.no_grad():
            actions = agent.batch_select_greedy_action(
                obss, deterministic=agent.act_deterministically
            )
        # o_{t+1}, r_{t+1}
        obss, rs, dones, infos = blank_env.step(actions)
        blank_episode_len += 1

        # Compute mask for done and reset
        if max_episode_len is None:
            resets = np.zeros(blank_env.env.num_envs, dtype=bool)
        else:
            resets = blank_episode_len == max_episode_len
        resets = np.logical_or(
            resets, [info.get("needs_reset", False) for info in infos]
        )
        ...
        #   5. reset the env to start a new episode
        # 3-5 are skipped when training is already finished.
        blank_episode_idx += end

        # Start new episodes if needed
        force_z = np.full((len(not_end),), -1)
        for i in range(len(not_end)):
            if not not_end[i]:
                if initial_z >= blank_env.n_skills:
                    initial_z = np.random.randint(0, blank_env.n_skills)

                have_reset[blank_env._z[i]] += 1

                force_z[i] = initial_z
                initial_z += 1

        blank_episode_len[end] = 0
        obss = blank_env.reset(not_end, force_z)

def train_agent_batch_with_evaluation_with_blanks(
    agent,
    env,
    steps,
    eval_n_steps,
