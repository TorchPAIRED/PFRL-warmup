...
def main():
    ...
    parser.add_argument(
        "--diayn-discriminator-type",
        type=str,
        default="ce",
        help="Either ce or bce",
    ...
    def make_batch_env(test, discriminator=None, force_no_diayn=False, is_evaluator=False, n_envs=None):
        ...
        if args.diayn_use and force_no_diayn is not True:
            if is_evaluator is False:
                ...
                env = DIAYNVecEval(env, args.diayn_n_skills, discriminator, args.diayn_concat_z_oh)
    ...
    if args.diayn_use:
        ...
        DiscriminatorFunction = CrossEntropyDiscriminator if args.diayn_discriminator_type == "ce" else BinaryEntropyDiscriminator
    ...
    if args.demo:
        ...
        class DIAYNEvalHook(EvaluationHook):
            ...
            def __call__(self, env, agent, evaluator, step, eval_stats, agent_stats, env_stats):
                ...
                env.rew_counter = torch.zeros((env.n_skills,))

        from custom_pfrl.train_agent_batch import train_agent_batch_with_evaluation_with_blanks
        from pfrl.experiments import train_agent_batch_with_evaluation
        train_agent_batch_with_evaluation(
            agent=agent,
            env=train_env,
            #blank_env=make_batch_env(test=False, discriminator=discriminator, n_envs=6),
            eval_env=eval_env,
            outdir=args.outdir,
            steps=args.steps,
